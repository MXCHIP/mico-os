...
download_app: $(STRIPPED_LINK_OUTPUT_FILE) display_map_summary download_bootloader kill_openocd
	...
	$(call CONV_SLASHES, $(OPENOCD_FULL_NAME)) -s $(SOURCE_ROOT) -f $(OPENOCD_CFG_PATH)interface/$(JTAG).cfg -f $(OPENOCD_CFG_PATH)$(HOST_OPENOCD)/$(HOST_OPENOCD).cfg -f $(OPENOCD_CFG_PATH)$(HOST_OPENOCD)/stm32l0_gdb_jtag.cfg -c "halt" -c "flash write_image unlock erase $(LINK_OUTPUT_FILE)" -c "flash write_image unlock erase mico-os/board/EML3047/mbed/boot.elf" -c shutdown $(DOWNLOAD_LOG) 2>&1 && $(ECHO) Download complete && $(ECHO_BLANK_LINE) || $(ECHO) Download failed
endif
