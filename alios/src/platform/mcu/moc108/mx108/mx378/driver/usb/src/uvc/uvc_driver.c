                                       ...
                                       const MUSB_EndpointDescriptor *pInVSEnd)
{
    ...
    hInVCPipe = MUSB_OpenPipe(hBus, &InVCEnd, NULL);
    if(!hInVCPipe)
    {
        MUSB_PRT("MUSB_OpenPipe VC fail\r\n");
        return FALSE;
    }
    ...
    if(!hInVSPipe)
    {
        MUSB_PRT("MUSB_OpenPipe VS fail\r\n");
    ...
    for(bIndex = 0; bIndex < pConfig->bNumInterfaces; bIndex++)
    {
        ...
        if(pIface && (MUSB_CLASS_VIDEO == pIface->bInterfaceClass))
        {
            ...
            for(bEnd = 0; bEnd < pIface->bNumEndpoints; bEnd++)
            {
                ...
                if(pEnd)
                {
                    if(UVC_SC_VIDEOCONTROL == bSubclass
                        && (MUSB_ENDPOINT_XFER_INT == (pEnd->bmAttributes & 
                                MUSB_ENDPOINT_XFERTYPE_MASK)))
                    {
                        pInVideoControlEp = pEnd;
                    }
                    
                    if(UVC_SC_VIDEOSTREAMING == bSubclass
                        && (MUSB_ENDPOINT_XFER_BULK == (pEnd->bmAttributes & 
                                MUSB_ENDPOINT_XFERTYPE_MASK)))
                    {
