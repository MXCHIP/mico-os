                         ...
                         int len)
{
    ...
    if (ioctl(drv->ioctl_sock, PRISM2_IOCTL_HOSTAPD, (unsigned long)&iwr) < 0)
    {
        return -1;
    }
...
static int hostap_ioctl_prism2param(void *priv, int param, int value)
{
    ...
    *i++ = value;
    ...
    if (ioctl(drv->ioctl_sock, PRISM2_IOCTL_PRISM2_PARAM, (unsigned long)&iwr) < 0)
    {
        return -1;
    }
...
static int hostap_set_ssid(void *priv, const u8 *buf, int len)
{
    ...
    if (ioctl(drv->ioctl_sock, SIOCSIWESSID, (unsigned long)&iwr) < 0)
    {
        return -1;
    }
...
static int hostap_get_we_version(struct hostap_driver_data *drv)
{
    ...
    if (ioctl(drv->ioctl_sock, SIOCGIWRANGE, (unsigned long)&iwr) < 0)
    {
        os_free(range);
        return -1;
    }
...
static int hostap_wireless_event_init(struct hostap_driver_data *drv)
{
    ...
    drv->netlink = 0; // netlink_init(cfg); wangzhiei
    if (drv->netlink == NULL)
    {
        //os_free(cfg);wangzhiei
        //return -1;wangzhiei
    }
...
static int hostap_set_freq(void *priv, struct hostapd_freq_params *freq)
{
    ...
    if (ioctl(drv->ioctl_sock, SIOCSIWFREQ, (unsigned long)&iwr) < 0)
    {
...
int hostap_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    *pos++ = 0;
    ...
    ret = hapd_intf_set_ap(beacon, bcn_len, params->head_len);
...
void wpa_handler_signal(void *arg)
{
	int sig = (int)arg;
...
{
    os_memcpy(bssid, assoc_ap.bssid, ETH_ALEN);
    return 0;
}
