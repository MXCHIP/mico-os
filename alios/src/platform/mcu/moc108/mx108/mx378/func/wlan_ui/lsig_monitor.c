...
{
    rts_tbl_num = 0;
    memset(&last_rts, 0 , sizeof(last_rts));
}

void rts_update(uint8_t *data, int8_t rssi, uint32_t time_ms)
{
    memcpy(last_rts.ra, &data[4], 12);
    last_rts.rssi = -rssi;
...
void beacon_update(uint8_t *bssid, uint8_t security)
{
    ...
    for(i=0; i<rts_tbl_num; i++) {
        if (rts_tbl[i].security != 0xFF)
            continue;
        if (memcmp(rts_tbl[i].ra, bssid, 6) == 0) {
...
static void report_lsig(int len, int8_t rssi, int index)
{
    ...
    uint16_t seq_frag;
    int i = 0;
    uint32_t time = mico_rtos_get_time();
    ...
    data[i++] = 0x88; // data
    if (rts_tbl[index].security == ENC_OPEN) {
        data[i++] = 0x1;
    } else {
    ...
    memcpy(&data[i], rts_tbl[index].ra, 6);
    i+=6;
    memcpy(&data[i], rts_tbl[index].ta, 6);
    i+=6;
    memset(&data[i], 0xFF, 6);
    i+=6;
    
    seq_frag = rts_tbl[index].seq;
    seq_frag <<= 4;
    memcpy(&data[i], &seq_frag, 2);
    i+=2;
    ...
    memset(&data[i], 0, 4);
    if (rts_tbl[index].security == ENC_CCMP) {
        data[i+3] = 0x20;
    } else if (rts_tbl[index].security == ENC_TKIP) {
        data[i+3] = 0x20;
        data[i] = 1;
    ...
    upload_monitor_data(data, len, &info);
}
...
void lsig_input(int len, int8_t rssi, uint32_t time_ms)
{
    ...
    for(i=0; i<rts_tbl_num; i++) {
        if (rts_match == 1) {
            if (memcmp(last_rts.ra, rts_tbl[i].ra, 12) == 0) {
    ...
    if (rts_tbl_num < MAX_RTS_TBL) {
        memcpy(rts_tbl[rts_tbl_num].ra, last_rts.ra, 12);
        rts_tbl[rts_tbl_num].security = 0xFF;
        rts_tbl[rts_tbl_num].rssi = rssi;
        rts_tbl[rts_tbl_num].last_time = mico_rtos_get_time();
