...
STRUCT_PACKED struct wpa_eapol_key {
	u8 type;
...
STRUCT_PACKED struct wpa_eapol_key_192 {
	u8 type;
	/* Note: key_info, key_length, and key_data_length are unaligned */
	u8 key_info[2]; /* big endian */
	u8 key_length[2]; /* big endian */
...
STRUCT_PACKED struct wpa_ie_hdr {
	u8 elem_id;
	u8 len;
...
STRUCT_PACKED struct rsn_ie_hdr {
...
STRUCT_PACKED struct rsn_error_kde {
	be16 mui;
	be16 error_type;
}GCC_PACKED;
...
STRUCT_PACKED struct wpa_igtk_kde {
	u8 keyid[2];
	u8 pn[6];
...
struct rsn_mdie {
STRUCT_PACKED 	u8 mobility_domain[MOBILITY_DOMAIN_ID_LEN];
	u8 ft_capab;
};
