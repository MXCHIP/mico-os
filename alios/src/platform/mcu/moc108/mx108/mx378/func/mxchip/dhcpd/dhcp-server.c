...
static mico_mutex_t dhcpd_mutex;
...
void dhcp_server(uint32_t data)
{
	int ret;
	static int one = 1;
	...
	mico_rtos_lock_mutex(&dhcpd_mutex);
	...
	dhcp_clean_sockets();
	mico_rtos_unlock_mutex(&dhcpd_mutex);
...
int dhcp_server_init(void *intrfc_handle)
{
	...
	if (dhcps.msg == NULL)
		return -1;
	ret = mico_rtos_init_mutex(&dhcpd_mutex);
	if (ret != 0)
		return -1;
...
out:
	...
	dhcps.msg = NULL;
	mico_rtos_deinit_mutex(&dhcpd_mutex);
	return ret;
}
...
int dhcp_free_allocations(void)
{
	...
	return mico_rtos_deinit_mutex(&dhcpd_mutex);
}
