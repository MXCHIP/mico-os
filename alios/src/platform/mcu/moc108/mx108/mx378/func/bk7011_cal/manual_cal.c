...
#define DEFAULT_TXID_THERMAL      (11111111)
...
const TXPWR_ST gtxpwr_tab_def_b[WLAN_2_4_G_CHANNEL_NUM] = {
    INIT_TXPWR_VALUE(9,  TXPWR_ELEM_INUSED),  // ch1
    INIT_TXPWR_VALUE(10, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(10, TXPWR_ELEM_INUSED),    
    INIT_TXPWR_VALUE(10, TXPWR_ELEM_INUSED),  // ch4
    INIT_TXPWR_VALUE(11, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(11, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(11, TXPWR_ELEM_INUSED),  // ch7
    INIT_TXPWR_VALUE(13, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(14, TXPWR_ELEM_INUSED), 
    INIT_TXPWR_VALUE(15, TXPWR_ELEM_INUSED),  // ch10
    INIT_TXPWR_VALUE(17, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(19, TXPWR_ELEM_INUSED),  // ch13
    INIT_TXPWR_VALUE(19, TXPWR_ELEM_INUSED),
};

const TXPWR_ST gtxpwr_tab_def_g[WLAN_2_4_G_CHANNEL_NUM] = {
    INIT_TXPWR_VALUE(17, TXPWR_ELEM_INUSED),  // ch1
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),    
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),  // ch4
    INIT_TXPWR_VALUE(19, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(19, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(19, TXPWR_ELEM_INUSED),  // ch7
    INIT_TXPWR_VALUE(21, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(22, TXPWR_ELEM_INUSED), 
    INIT_TXPWR_VALUE(22, TXPWR_ELEM_INUSED),  // ch10
    INIT_TXPWR_VALUE(24, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(25, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(25, TXPWR_ELEM_INUSED),  // ch13
    INIT_TXPWR_VALUE(25, TXPWR_ELEM_INUSED),
};

const TXPWR_ST gtxpwr_tab_def_n_20[WLAN_2_4_G_CHANNEL_NUM] = {
    INIT_TXPWR_VALUE(17+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),  // ch1 
    INIT_TXPWR_VALUE(18+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(18+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(18+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),  // ch4 
    INIT_TXPWR_VALUE(19+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(19+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(19+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),  // ch7 
    INIT_TXPWR_VALUE(21+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(22+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(22+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),  // ch10
    INIT_TXPWR_VALUE(24+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(25+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
    INIT_TXPWR_VALUE(25+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),  // ch13
    INIT_TXPWR_VALUE(25+MOD_DIST_G_BW_N20, TXPWR_ELEM_INUSED),         
};
...
const TXPWR_ST gtxpwr_tab_def_n_40[WLAN_2_4_G_CHANNEL_NUM] = {
    INIT_TXPWR_VALUE(11, TXPWR_ELEM_INUSED),  
    INIT_TXPWR_VALUE(11, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(11, TXPWR_ELEM_INUSED),  // ch3 
    INIT_TXPWR_VALUE(12, TXPWR_ELEM_INUSED),  
    INIT_TXPWR_VALUE(13, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(13, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(13, TXPWR_ELEM_INUSED),  // ch7
    INIT_TXPWR_VALUE(15, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(16, TXPWR_ELEM_INUSED), 
    INIT_TXPWR_VALUE(16, TXPWR_ELEM_INUSED),  
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),  // ch11
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),  
    INIT_TXPWR_VALUE(18, TXPWR_ELEM_INUSED),
};
...
int manual_cal_get_txpwr(UINT32 rate, UINT32 channel, UINT32 bandwidth, UINT32 *pwr_gain)
{
    ...
    if((rate <= MAX_RATE_FOR_B_5M) || (rate == MAX_RATE_FOR_B)) {
        ...
        if(rate <= MAX_RATE_FOR_N && rate >= MAX_RATE_MIX_N) {  
            ...
            MCAL_FATAL("Manual cal wrong rate with BW40:%d\r\n", rate);
            return 0;
        }
    }

    if(GET_TXPWR_FLAG(txpwr_tab_ptr) == TXPWR_ELEM_INUSED) {
        
        *pwr_gain = GET_TXPWR_GAIN(txpwr_tab_ptr);
        MCAL_PRT("get txpwrtab gain:%d\r\n", *pwr_gain);

        *pwr_gain &= GAIN_MASK;
        *pwr_gain = GAIN_MASK - *pwr_gain;
...
TXPWR_IS_RD manual_cal_txpwr_tab_is_fitted(void)
{
    ...
    UINT32 i;
        ...
        for(i=0; i<WLAN_2_4_G_CHANNEL_NUM; i++){
            txpwr_tab_ptr = &gtxpwr_tab_b[i];
        if(GET_TXPWR_FLAG(txpwr_tab_ptr) == TXPWR_ELEM_UNUSED) {
        ...
        txpwr_tab_ptr = &gtxpwr_tab_n_40[i];
        if(GET_TXPWR_FLAG(txpwr_tab_ptr) == TXPWR_ELEM_UNUSED){
                MCAL_WARN("txpwr table n[%d] is unused\r\n", i);
                break;
            }
        }
        if(i == WLAN_2_4_G_CHANNEL_NUM)
            is_ready |= TXPWR_TAB_N_RD;
...
UINT32 manual_cal_fitting_txpwr_tab(void)
{
    ...
    if((GET_TXPWR_FLAG(&tab_ptr[0]) == TXPWR_ELEM_UNUSED)  
      ||(GET_TXPWR_FLAG(&tab_ptr[6]) == TXPWR_ELEM_UNUSED)
    ...
    if((GET_TXPWR_FLAG(&tab_ptr[2]) == TXPWR_ELEM_UNUSED)   // ch3
      ...
      ||(GET_TXPWR_FLAG(&tab_ptr[10]) == TXPWR_ELEM_UNUSED) ){  // ch11
        MCAL_WARN("txpwr table for n40 fitting failed!, ch3 ch7 ch11 unused\r\n");
    } else {
...
static UINT32 manual_cal_search_opt_tab(UINT32 *len)
{
    ...
    *len = 0;
    ...
    ddev_read(flash_handle, (char *)&head, sizeof(TXPWR_ELEM_ST), TXPWR_TAB_FLASH_ADDR);
...
static UINT8 manual_cal_update_flash_area(UINT32 addr, char *buf, UINT32 len)
{
    DD_HANDLE flash_handle;
    UINT32 param, param1, status, read_len = 0, read_addr= 0, i;
    UINT8 ret = 0;
    UINT8 *read_buf = NULL;

    if(addr < TXPWR_TAB_FLASH_ADDR || (addr > TXPWR_TAB_FLASH_ADDR + (4*1024))){
        MCAL_FATAL("write flash addr error:%08x\r\n", addr);
        return 1;
    }
    if(len == 0)
        return 0;

    if(addr % BK_FLASH_SECTOR_SIZE) {
         read_len += (addr % BK_FLASH_SECTOR_SIZE);   
    }
    read_len += len;
    if((addr+len) % BK_FLASH_SECTOR_SIZE) {
         read_len += (BK_FLASH_SECTOR_SIZE - (addr % BK_FLASH_SECTOR_SIZE));   
    }
    read_addr = addr - (addr % BK_FLASH_SECTOR_SIZE);

    //MCAL_PRT("read first, addr:%08x, len:%d\r\n", read_addr, read_len);

    read_buf = (UINT8*)os_malloc(read_len); 
    if(!read_buf){
    ...
    status = ddev_read(flash_handle, read_buf, read_len, read_addr);
    if(status != FLASH_SUCCESS) {
        MCAL_FATAL("cann't read flash before write\r\n");
        ret = 1;
    }

    os_memcpy(read_buf+(addr % BK_FLASH_SECTOR_SIZE), buf, len);

	param1 = 2 |(0<<8);
    ddev_control(flash_handle, CMD_FLASH_WRITE_SR, &param1);

    for(i=0; i<(read_len/BK_FLASH_SECTOR_SIZE); i++){
        param1 = read_addr + i;
        ddev_control(flash_handle, CMD_FLASH_ERASE_SECTOR, &param1);
    }

    status = ddev_write(flash_handle, (char *)read_buf, read_len, read_addr);
...
static UINT8 manual_cal_read_flash(UINT32 addr, char *buf, UINT32 len)
{
    UINT32 status;
    UINT8 ret = 0;
    DD_HANDLE flash_handle;
    
    if(addr < 0xab000 || (addr > 0xab000 + (4*1024))){
...
UINT32 manual_cal_load_txpwr_tab_flash(void)
{
    ...
    addr_start = manual_cal_search_txpwr_tab(TXPWR_TAB_TAB, TXPWR_TAB_FLASH_ADDR); 
    if(!addr_start) {
    ...
    if(is_ready_flash & TXPWR_TAB_G_RD) {
        ...
        // for n20    
        addr = manual_cal_search_txpwr_tab(TXPWR_TAB_DIF_GN_ID, addr_start);
        if(addr) {
            ddev_read(flash_handle, (char *)&head, sizeof(TXPWR_ELEM_ST), addr);
...
UINT32 manual_cal_load_default_txpwr_tab(UINT32 is_ready_flash)
{
    ...
    if(!(is_ready_flash & TXPWR_TAB_G_RD)) {
        ...
        os_memcpy(gtxpwr_tab_n, gtxpwr_tab_def_n_20, sizeof(gtxpwr_tab_def_n_20));
        MCAL_WARN("Load default txpwr for n20:%p\r\n", gtxpwr_tab_def_n_20);
    }
...
int manual_cal_save_txpwr_tab_to_flash(void)
{
    ...
    txpwr_len = sizeof(TAG_TXPWR_ST) + sizeof(TAG_ENABLE_ST) 
            + 3*sizeof(TAG_TXPWR_TAB_ST) + sizeof(TAG_DIF_ST);
    ...
    if(status) {
        ...
        if(!addr_start) {
            ...
            // copy flash
            ddev_read(flash_handle, (char *)buf, flash_len, TXPWR_TAB_FLASH_ADDR);
            ddev_close(flash_handle);
    ...
    // for tag TXPWR_TAB_DIF_GN_ID
    tag_dif_ptr = (TAG_DIF_PTR)(txpwr_buf);  
    tag_dif.head.type = TXPWR_TAB_DIF_GN_ID;
...
int manual_cal_save_chipinfo_tab_to_flash(void)
{
    ...
    chipinfo_len = sizeof(TAG_TXPWR_ST) + sizeof(TAG_COMM_ST) + sizeof(TAG_CHIPMAC_ST)
    ...
    // Has TLV Header?
    if(status) {
        addr_start = manual_cal_search_txpwr_tab(TXID, TXPWR_TAB_FLASH_ADDR); 
        if(!addr_start) {
    ...
    {
        UINT32 i=0;
        MCAL_PRT("\r\nshow chipinfo tags before write flash:\r\n");
        for(i=0; i<len; i++) {
            MCAL_PRT("%02x,", buf[i]);
            if((i+1)%16 == 0)
                MCAL_PRT("\r\n");
    ...
    manual_cal_update_flash_area(TXPWR_TAB_FLASH_ADDR, (char *)buf, len);
    os_free(buf);
 
    return 1;
}

/* MXCHIP save MAC address */
#if 0
...
int manual_cal_write_macaddr_to_flash(UINT8 *mac_ptr)
{
    ...
    if(addr) {
        ddev_read(flash_handle, (char *)&head, sizeof(TXPWR_ELEM_ST), addr);
        manual_cal_update_flash_area(addr + sizeof(TXPWR_ELEM_ST), (char *)mac_ptr, 6); //0: sucess, 1 failed
    }else {
        MCAL_FATAL("No MAC id found in txid header \r\n");
        return 0;
    }
