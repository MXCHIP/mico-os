...
static const u8 WPA_OUI23A_TYPE[] = { 0x00, 0x50, 0xf2, 1 };
...
struct ap_info *zconfig_aplist;
/* aplist num, less than MAX_APLIST_NUM */
u8 zconfig_aplist_num;
                                        ...
                                        u8 auth, u8 pairwise_cipher, u8 group_cipher)
{
    ...
    */
    ...
    log("[%d] ssid:%s, mac:%02x%02x%02x%02x%02x%02x, chn:%d, auth:%s, %s, %s\r\n",
        ...
        zconfig_encry_str(group_cipher));
    /*
...
ATTR int ieee80211_data_extract(u8 *in, int len, int link_type, struct parser_res *res)
{
    ...
    if (ieee80211_is_beacon(fc)) {
        u16 ieoffset = offsetof(struct ieee80211_mgmt, u.beacon.variable);
        const u8 *alibaba_ie = cfg80211_find_vendor_ie(WLAN_OUI_ALIBABA, WLAN_OUI_TYPE_ALIBABA,
                                                       in + ieoffset, len - ieoffset);
        ret = zconfig_extract_apinfo_from_beacon(in, len);
        if (ret > 0) {
            alink_type = ALINK_DEFAULT_SSID;
            goto output;
        }
        if (alibaba_ie) {
            //log("got a %s fc=%x\r\n", "beacon", fc);
            alink_type = ALINK_ROUTER;
            res->u.ie.alink_ie_len = len - (alibaba_ie - in);
            res->u.ie.alink_ie = (u8 *)alibaba_ie;
            goto output;
        }
        goto drop;
...
#define ACTION_OFFSET               (24)        //from header to category
        u8 *category = in + ACTION_OFFSET;
        u8 vendor[] = {0x7F, 0, 0x17, 0xF2, 0x08, 0x10, 0};
        if (memcmp(category, vendor, sizeof(vendor))) {
            goto drop;
        }
        alink_type = ALINK_ACTION;
        res->u.action.data_len = len - ACTION_OFFSET;
        res->u.action.data = (u8 *)category;
        goto output;
    }
