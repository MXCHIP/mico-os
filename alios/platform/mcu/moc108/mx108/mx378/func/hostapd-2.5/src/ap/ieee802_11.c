			    ...
			    const u8 *ies, size_t ies_len)
{
	...
	rlen = IEEE80211_HDRLEN + sizeof(reply->u.auth) - VARIABLE_ARRAY_SPARE + ies_len;
	buf = os_zalloc(rlen);
	if (buf == NULL)
		return;
			...
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.auth) - VARIABLE_ARRAY_SPARE) {
	...
	if (len >= IEEE80211_HDRLEN + sizeof(mgmt->u.auth) - VARIABLE_ARRAY_SPARE + 2 + WLAN_AUTH_CHALLENGE_LEN 
	...
	case WLAN_AUTH_FT:
		...
		wpa_ft_process_auth(sta->wpa_sm, mgmt->bssid,
				    ...
				    sizeof(mgmt->u.auth) + VARIABLE_ARRAY_SPARE,
			    ...
			    size_t ies_len)
{
	...
	send_len += sizeof(reply->u.assoc_resp) - VARIABLE_ARRAY_SPARE;
			 ...
			 int reassoc)
{
	...
	if (len < IEEE80211_HDRLEN + (reassoc ? (sizeof(mgmt->u.reassoc_req) - VARIABLE_ARRAY_SPARE) :
				      (sizeof(mgmt->u.assoc_req) - VARIABLE_ARRAY_SPARE))) {
	...
	if (reassoc) {
		...
		left = len - (IEEE80211_HDRLEN + sizeof(mgmt->u.reassoc_req) - VARIABLE_ARRAY_SPARE);
		...
		wpa_printf(MSG_DEBUG, "association request: STA=" MACSTR
			   ...
			   MAC2STR(mgmt->sa), capab_info, listen_interval,
			   seq_ctrl, (fc & WLAN_FC_RETRY) ? " retry" : "");
		left = len - (IEEE80211_HDRLEN + sizeof(mgmt->u.assoc_req) - VARIABLE_ARRAY_SPARE);
			    ...
			    const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.disassoc) - VARIABLE_ARRAY_SPARE) {
			  ...
			  const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.deauth) - VARIABLE_ARRAY_SPARE) {
			  ...
			  struct hostapd_frame_info *fi)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.beacon) - VARIABLE_ARRAY_SPARE) {
	...
	(void) ieee802_11_parse_elems(mgmt->u.beacon.variable,
				      len - (IEEE80211_HDRLEN +
					     sizeof(mgmt->u.beacon) - VARIABLE_ARRAY_SPARE), &elems,
				      0);
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	if (len < IEEE80211_HDRLEN + (reassoc ? (sizeof(mgmt->u.reassoc_resp) - VARIABLE_ARRAY_SPARE) :
				      (sizeof(mgmt->u.assoc_resp) - VARIABLE_ARRAY_SPARE))) {
