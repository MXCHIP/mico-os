...
void vTaskForceAwake ( xTaskHandle pxTaskToResume )
{
	...
	taskENTER_CRITICAL();
	{
		if( pxTCB->xEventListItem.pvContainer )
		{
			...
			vListRemove( ( xListItem * )&( pxTCB->xEventListItem ) );
		...
		do
		{
			...
			#else
			{
				usStackRemaining = usTaskCheckFreeStackSpace( ( unsigned char * ) pxNextTCB->pxStack );
                        ...
                        _priority = pxNextTCB->uxPriority;
			sprintf( pcStatusString, ( char * ) "%-32s %c\t%u\t%u\t%u\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) ((configMAX_PRIORITIES-1) - _priority), usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber );
...
/*-----------------------------------------------------------*/
#ifdef MXCHIP
static tskTCB* getTcbWithinSingleList(  xList *pxList, int tcbNumb )
{
tskTCB *pxNextTCB, *pxFirstTCB;
	...
	{
		listGET_OWNER_OF_NEXT_ENTRY( pxNextTCB, pxList );
		if ( pxNextTCB->uxTCBNumber == tcbNumb)
            return pxNextTCB;
...
/* Mico APIs to get the callstack of a task. */
int vTaskCallstack(int tcbNumb, unsigned int cstack[])
{
    unsigned portBASE_TYPE uxQueue;
    tskTCB *pTcb;
    portSTACK_TYPE *stackptr, *stackend, *stacktop;
    unsigned int p;
    int i, j, stacklen;
        /* This is a VERY costly function that should be used for debug only.
        It leaves interrupts disabled for a LONG time. */
    ...
    vTaskSuspendAll();
    {
        ...
        do
        {
            ...
            if( listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) == pdFALSE )
            {
                pTcb = getTcbWithinSingleList( ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), tcbNumb );
                if (pTcb != NULL)
                    goto FOUND;
            }
        }while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );
        ...
        if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
        {
            pTcb = getTcbWithinSingleList( ( xList * ) pxDelayedTaskList, tcbNumb );
            if (pTcb != NULL)
                goto FOUND;
        }
        ...
        if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
        {
            pTcb = getTcbWithinSingleList(( xList * ) pxOverflowDelayedTaskList, tcbNumb );
            if (pTcb != NULL)
                goto FOUND;
        }
	...
	#if( INCLUDE_vTaskDelete == 1 )
        {
            if( listLIST_IS_EMPTY( &xTasksWaitingTermination ) == pdFALSE )
            {
                pTcb = getTcbWithinSingleList(&xTasksWaitingTermination, tcbNumb );
                if (pTcb != NULL)
                    goto FOUND;
            }
        }
	...
	#if ( INCLUDE_vTaskSuspend == 1 )
        {
            if( listLIST_IS_EMPTY( &xSuspendedTaskList ) == pdFALSE )
            {
                pTcb = getTcbWithinSingleList(&xSuspendedTaskList, tcbNumb );
                if (pTcb != NULL)
                    goto FOUND;
            }
        }
...
FOUND:
    ...
    vTaskGetStackInfo(pTcb, (void**)&stackptr, (void**)&stacktop, (void**)&stackend);
    if (stacktop < stackptr)
        return -1;
    stacklen = (stacktop - stackptr)/4;
    
    i = j = 0;
    while(j < stacklen) {
        p = (unsigned int)stacktop[-j];
        j++;
        if ((p & 0xFF000000) == 0x08000000) {
            cstack[i++] = p;
            if (i == 10)
                break;
        }
