...
# init4390     - run initialisation for the 4390
###############################################################################
proc sflash_read_file { filename partition srcAddress PlatBusDebug length } {
    global COMMAND_ERASE COMMAND_INITIAL_VERIFY COMMAND_WRITE COMMAND_POST_WRITE_VERIFY buffer_size COMMAND_READ data_loc

    sflash_init $PlatBusDebug
    set file_name [format "tmp/readtmp.%d.bin" $::swdaddr]
    set temp_file $file_name
    exec echo -n > $filename
    ...
    while { $pos < $length } {
        ...
        program_sflash "" $pos $readsize $partition [expr $srcAddress + $pos] $read_command_val
