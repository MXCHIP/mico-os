...
OSStatus platform_flash_init( const platform_flash_t *peripheral )
{
  ...
  }else if(peripheral->flash_type == FLASH_TYPE_EEPROM){
    err = ieeprom_init();
    require_noerr(err, exit);
  }
...
OSStatus platform_flash_erase( const platform_flash_t *peripheral, uint32_t start_address, uint32_t end_address )
{
  ...
  if( peripheral->flash_type == FLASH_TYPE_EMBEDDED ){
    ...
    err = ieeprom_erase( start_address, end_address );
    require_noerr(err, exit);
  }
...
OSStatus platform_flash_read( const platform_flash_t *peripheral, volatile uint32_t* start_address, uint8_t* data ,uint32_t length  )
{
  ...
  if( peripheral->flash_type == FLASH_TYPE_EMBEDDED || peripheral->flash_type == FLASH_TYPE_EEPROM ){
...
#define LL_SYSCFG_BANKMODE_BANK1           (uint32_t)0x00000000U
int get_passive_firmware(void)
{
    if(LL_SYSCFG_BANKMODE_BANK1 == LL_SYSCFG_GetFlashBankMode())
    {
      return 2;
    }
    /*boot from bank2*/
