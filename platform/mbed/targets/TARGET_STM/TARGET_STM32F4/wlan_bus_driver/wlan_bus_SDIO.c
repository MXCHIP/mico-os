...
{
    [BUS_READ]  = SDIO_TRANSFER_DIR_TO_SDIO, //SDIO_TransferDir_ToSDIO,
...
{
    if ( platform_gpio_input_get( &wifi_control_pin_drivers[WIFI_PIN_SDIO_IRQ] ) == true) //SDIO INT pin is high
        return false;
    else
...
OSStatus host_platform_bus_init( void )
{
    ...
    NVIC_SetVector(SDIO_IRQn, (uint32_t)&sdio_irq);
    NVIC_SetPriority( SDIO_IRQn, 2 );
    NVIC_EnableIRQ(SDIO_IRQn);
    NVIC_SetVector(DMA2_Stream3_IRQn, (uint32_t)&dma_irq);
    ...
    platform_gpio_init      ( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_0], &wifi_control_pins[WIFI_PIN_BOOTSTRAP_0], OUTPUT_PUSH_PULL );
    platform_gpio_output_low( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_0] );
#endif //MICO_WIFI_USE_GPIO_FOR_BOOTSTRAP_0

#ifdef MICO_WIFI_USE_GPIO_FOR_BOOTSTRAP_1
    platform_gpio_init          ( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_1], &wifi_control_pins[WIFI_PIN_BOOTSTRAP_1], OUTPUT_PUSH_PULL );
#ifdef MICO_WIFI_USE_GPIO_FOR_BOOTSTRAP_1_HIGH_FOR_SDIO
    platform_gpio_output_high   ( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_1] );
#else
    platform_gpio_output_low    ( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_1] );
    ...
    sdio_init_structure.ClockDiv            = (uint8_t) 120; /* 0x78, clock is taken from the high speed APB bus ; */ /* About 400KHz */
    sdio_init_structure.ClockEdge           = SDIO_CLOCK_EDGE_RISING;
    sdio_init_structure.ClockBypass         = SDIO_CLOCK_BYPASS_DISABLE;
    sdio_init_structure.ClockPowerSave      = SDIO_CLOCK_POWER_SAVE_ENABLE;
    sdio_init_structure.BusWide             = SDIO_BUS_WIDE_1B;
...
OSStatus host_platform_bus_deinit( void )
{
    ...
    {
        HAL_GPIO_DeInit((GPIO_TypeDef *)(GPIOA_BASE + (GPIOB_BASE-GPIOA_BASE) * STM_PORT( wifi_sdio_pin_drivers[a].gpio.pin)), STM_PIN(wifi_sdio_pin_drivers[a].gpio.pin));
    }

#ifdef MICO_WIFI_USE_GPIO_FOR_BOOTSTRAP_0
    platform_gpio_deinit( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_0] );
#endif
#ifdef MICO_WIFI_USE_GPIO_FOR_BOOTSTRAP_1
    platform_gpio_deinit( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_1] );
#endif
...
static void sdio_prepare_data_transfer( bus_transfer_direction_t direction, sdio_block_size_t block_size, /*@unique@*/ uint8_t* data, uint16_t data_size ) /*@modifies dma_data_source, user_data, user_data_size, dma_transfer_size@*/
{
    ...
    SDIO->DCTRL  = (uint32_t)sdio_get_blocksize_dctrl(block_size) | bus_direction_mapping[(int)direction] | SDIO_TRANSFER_MODE_BLOCK | SDIO_DPSM_ENABLE | (1 << 3) | (1 << 11);
...
void host_platform_enable_high_speed_sdio( void )
{
    ...
    sdio_init_structure.BusWide             = SDIO_BUS_WIDE_1B;
#else
    sdio_init_structure.BusWide             = SDIO_BUS_WIDE_4B;
#endif
    sdio_init_structure.HardwareFlowControl = SDIO_HARDWARE_FLOW_CONTROL_DISABLE;
