...
OSStatus mico_gpio_init( mico_gpio_t gpio, mico_gpio_config_t configuration )
{
  ...
  platform_gpio_drivers[gpio].port = platform_gpio_pins[gpio].port;
  platform_gpio_drivers[gpio].config = configuration;

  return (OSStatus) hal_gpio_init( &platform_gpio_drivers[gpio] );
}
...
OSStatus mico_gpio_output_high( mico_gpio_t gpio )
{
  if ( gpio >= MICO_GPIO_NONE )
    return kUnsupportedErr;
  return (OSStatus) hal_gpio_output_high( &platform_gpio_drivers[gpio] );
}
...
OSStatus mico_gpio_output_low( mico_gpio_t gpio )
{
  if ( gpio >= MICO_GPIO_NONE )
    return kUnsupportedErr;
  return (OSStatus) hal_gpio_output_low( &platform_gpio_drivers[gpio] );
}
...
OSStatus mico_gpio_output_trigger( mico_gpio_t gpio )
{
  if ( gpio >= MICO_GPIO_NONE )
    return kUnsupportedErr;
  return (OSStatus) hal_gpio_output_toggle( &platform_gpio_drivers[gpio] );
}
                         ...
                         uint32_t inBufferLength )
{
    ...
    require_action_quiet( end_addr < partition_info->partition_start_addr + mico_partitions[partition].partition_length,
                          exit, err = kParamErr );
