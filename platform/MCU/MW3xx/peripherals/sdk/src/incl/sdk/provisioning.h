...
enum prov_event {
	...
	*/
	PROV_ABORTED, /* unused */
	/** This event is sent to the application on successful POST on
...
typedef int (*nw_save_cb_t)(struct wlan_network *);
 ...
 *
 * In multi model provisioning, device enters in SMC mode in which it sniffs
 ...
 *
 * In multi model provisioning, when device receives micro AP auth request, it
