...
#ifdef CONFIG_WPA2_ENTP
	char identity[WLAN_NETWORK_NAME_MAX_LENGTH];
#endif
 ...
 *
 *  \return WM_SUCCESS if successful.
 *  \return -WM_E_INVAL if \a cb is NULL.
 *  \return -WM_E_NOMEM if memory is not available for scan parameters.
 *  \return WLAN_ERROR_STATE if the WLAN Connection Manager was
 ...
 *  while another scan is in progress.
 *
 *  \param[in] wlan_scan_param  A pointer to the \ref wlan_scan_params_v2_t
 *	       structure holding a pointer to function that will be called
...
/** Turn on Power Save for micro-AP (Inactivity Timeout)
 *
 * This puts the micro-AP in a power-save mode after the inactivity
 * timeout. Please refer to the wireless chips documentation for more details
 * about the parameters.
 *
 * \note This call is asynchronous. System will enter power-save only when all
 * requisite conditions are met.
 *
 * \param[in] ctrl_bitmap disable (0) or enable (1) protection frame Tx before
 * power save
 * \param[in] inactivity_to inactivity timeout in microseconds
 * \param[in] min_sleep minimum sleep duration in microseconds
 * \param[in] max_sleep maximum sleep duration in microseconds
 * \param[in] min_awake minimum awake duration in microseconds
 * \param[in] max_awake maximum awake duration in microseconds
 *
 * \return WM_SUCCESS if the call was successful.
 * \return -WM_E_INVAL if given parameters are not valid.
 * \return WLAN_ERROR_STATE if the call was made in a state where such 
 * 	   an operation is illegal.
 */
...
/** Turn off Power Save for micro-AP (Inactivity Timeout)
 *
 * This puts the micro-AP out of the inactivity timeout based power save state.
 *
 * \note This call is asynchronous. System will exit power-save only when all
 * requisite conditions are met.
 *
 * \return WM_SUCCESS if the call was successful.
 * \return WLAN_ERROR_STATE if the call was made in a state where such an
 ...
 * \param[in] radio_type Radio type 2.4 or 5 GHz.
 ...
 *		     10 and 11 = reserved
 *
 * \note To sniff packets in 40 MHz channel bandwidth, specify secondary channel
 * offset above/below in radio type.
 *
 ...
 * \param[in] duration The duration for which to configure
 *
 * \return WM_SUCCESS on success or error code.
 *
 */
...
#ifdef CONFIG_WPA2_ENTP
int wpa2_connect(struct wlan_network *wpa2_network);
#endif
