...
os_thread_t sdio_cmd_type;
static os_thread_stack_define(sdio_cmd_type_stack, 512);
os_semaphore_t sdio_cmd_type_sem;
static int sdio_cmd_type_flag;
#endif

void sdio_dumpregs()
{
	sdio_l("SYSADDR: 0x%08x\r\n", SDIO->SYSADDR.WORDVAL);
	sdio_l("BLK_CNTL: 0x%08x\r\n", SDIO->BLK_CNTL.WORDVAL);
	sdio_l("ARG: 0x%08x\r\n", SDIO->ARG.WORDVAL);
	sdio_l("CMD_XFRMD: 0x%08x\r\n", SDIO->CMD_XFRMD.WORDVAL);
	sdio_l("RESP0: 0x%08x\r\n", SDIO->RESP[0].WORDVAL);
	sdio_l("RESP1: 0x%08x\r\n", SDIO->RESP[1].WORDVAL);
	sdio_l("RESP2: 0x%08x\r\n", SDIO->RESP[2].WORDVAL);
	sdio_l("RESP3: 0x%08x\r\n", SDIO->RESP[3].WORDVAL);
	...
	sdio_l("CNTL1: 0x%08x\r\n", SDIO->CNTL1.WORDVAL);
	sdio_l("CNTL2: 0x%08x\r\n", SDIO->CNTL2.WORDVAL);
	sdio_l("I_STAT: 0x%08x\r\n", SDIO->I_STAT.WORDVAL);
	sdio_l("I_STAT_EN: 0x%08x\r\n", SDIO->I_STAT_EN.WORDVAL);
	sdio_l("I_SIG_EN: 0x%08x\r\n", SDIO->I_SIG_EN.WORDVAL);
	sdio_l("CAP0: 0x%08x\r\n", SDIO->CAP0.WORDVAL);
	sdio_l("VER: 0x%08x\r\n", SDIO->VER.WORDVAL);
...
#ifdef CONFIG_BT_SUPPORT
		os_semaphore_put(&sdio_cmd_type_sem);
#else
		if (cdint_cb)
			cdint_cb(0);
		if (cdint_cb1)
...
static int sdio_check_status(int cnt, int dat)
{
	...
	while (cmd || dat) {
		...
		if (((struct I_STAT *)&i_stat)->ERRINT) {
			...
			sdio_l("Command/Data Transfer error cmd = %d dat = %d"
		...
		if (!cnt--) {
			sdio_l("Timeout waiting for Command/Data to complete "
...
static int sdio_card_init()
{
	...
	sdio_l("Card Version - (0x%x)", resp & 0xff);
...
#ifdef CONFIG_BT_SUPPORT
	int total_wait_time = 5 * 1000;
	int check_interval = 200;
	int num_iterations = total_wait_time / check_interval;
	sdio_cmd_type_flag = 1;
	os_semaphore_put(&sdio_cmd_type_sem);
	while (sdio_cmd_type_flag == 1 && --num_iterations) {
		os_thread_sleep(os_msec_to_ticks(check_interval));
	}
	sdio_cmd_type_flag = 0;
	os_thread_delete(&sdio_cmd_type);
	os_thread_sleep(os_msec_to_ticks(50));

	os_mutex_delete(&sdio_cmd_type_sem);
#endif
...
#ifdef CONFIG_BT_SUPPORT
void sdio_cmd_type_thread(os_thread_arg_t arg)
{
	uint8_t value;
	while (1) {
		os_semaphore_get(&sdio_cmd_type_sem, OS_WAIT_FOREVER);
		if (sdio_cmd_type_flag == 1)
			break;
		value = read_sdio_function_recvd();
		if (value & 0x02) {
			if (cdint_cb)
				cdint_cb(0);
		}
...
int sdio_drv_init(void (*cd_int)(int))
{
...
#ifdef CONFIG_BT_SUPPORT
	ret = os_semaphore_create(&sdio_cmd_type_sem,
				     "sdio-cmd-type-sem");
	if (ret != WM_SUCCESS)
		return ret;
	os_semaphore_get(&sdio_cmd_type_sem, OS_WAIT_FOREVER);

	 ret = os_thread_create(&sdio_cmd_type,
			       "sdio-cmd-type", sdio_cmd_type_thread, 0,
			       &sdio_cmd_type_stack, OS_PRIO_3);
	if (ret != WM_SUCCESS)
		return ret;
#endif
	...
	if (board_card_detect() == false) {
		sdio_e("Card not detected");
		return -WM_FAIL;
	} else
		sdio_l("Card detected");

	if (sdio_card_reset() == false) {
		sdio_e("Card reset failed");
		return -WM_FAIL;
	} else
		sdio_l("Card reset successful");
	...
	if (sdio_card_init() == false) {
		...
		sdio_l("Card initialization successful");
