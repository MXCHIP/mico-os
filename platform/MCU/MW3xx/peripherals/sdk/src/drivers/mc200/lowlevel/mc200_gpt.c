 ...
 */
const static uint32_t gptAddr[4] = {GPT0_BASE, GPT1_BASE, GPT2_BASE, GPT3_BASE};
...
{  
  ...
  GPTx->CLK_CNTL.BF.CLK_SRC = gptConfig->clockSrc;
   
  /* set clock prescaler */
  GPTx->CLK_CNTL.BF.CLK_PRE = gptConfig->clockPrescaler;

  /* set clock divider */
  GPTx->CLK_CNTL.BF.CLK_DIV = gptConfig->clockDivider;

  /************************Configure the counter *****************************/

  /* set upp value */
  GPTx->CNT_UPP_VAL.BF.UPP_VAL = gptConfig->uppVal;
  
  /* set counter value update mode */
  GPTx->CNT_CNTL.BF.CNT_UPDT_MOD = gptConfig->cntUpdate;
}
...
void GPT_ChannelReset(GPT_ID_Type gptID, uint32_t resetChannelNo)
{
  ...
  GPTx->USER_REQ.WORDVAL |= (((resetChannelNo) & 0x3F)<<8);    
}
...
void GPT_InputConfig(GPT_ID_Type gptID, GPT_InputConfig_Type* gptInputConfig)
{
  ...
  GPTx->IC_CNTL.BF.IC_DIV = gptInputConfig->sampleClkDivider;

  /* select input filtering */
  GPTx->IC_CNTL.BF.IC_WIDTH = gptInputConfig->trigFilter;
}
                           ...
                           GPT_ChannelFunc_Type channelFunc)
{
  ...
  GPTx->CH[channelID].CHX_CNTL.BF.CH_IO = channelFunc;
}
...
{  
  ...
  GPTx->CH[channelID].CHX_CNTL.BF.IC_EDGE = edgeSel;
}
                          ...
                          GPT_ChannelOutputConfig_Type* gptChOutConfig)
{
  ...
  GPTx->CH[channelID].CHX_CNTL.BF.POL = gptChOutConfig->polarity;
  
  /************************set channel match values*****************************/

  /* set CMR0 */
  GPTx->CH[channelID].CHX_CMR0.BF.CMR0 = gptChOutConfig->cntMatchVal0;
  
  /* set CMR1 */
  GPTx->CH[channelID].CHX_CMR1.BF.CMR1 = gptChOutConfig->cntMatchVal1;
  
  /* update them to internal shadow registers */
  GPTx->USER_REQ.WORDVAL |= (0x00010000 << channelID);
}
...
{	
  ...
  GPTx->CNT_UPP_VAL.BF.UPP_VAL = uppVal;
}
...
{  
  ...
  /* set channel CMR0 */
  GPTx->CH[channelID].CHX_CMR0.BF.CMR0 = cmr0;
...
void GPT_SetChannelMatchVal1(GPT_ID_Type gptID, GPT_ChannelNumber_Type channelID, uint32_t cmr1)
{
  ...
  /* set channel CMR1 */
  GPTx->CH[channelID].CHX_CMR1.BF.CMR1 = cmr1;
...
uint32_t GPT_GetCaptureVal(GPT_ID_Type gptID, GPT_ChannelNumber_Type channelID)
{
  ...
  /* read the value */
  capValue = GPTx->CH[channelID].CHX_CMR0.BF.CMR0;
...
{  
  ...
  /* read the value */
  cntValue = GPTx->CNT_VAL.BF.CNT_VAL;
...
{  
  ...
  if(ENABLE == state)
  {
    GPTx->DMA_CNTL_EN.WORDVAL |= (1<<dmaChannel);
  }
  if(DISABLE == state)
  {
    GPTx->DMA_CNTL_EN.WORDVAL &= (~(1<<dmaChannel));
                          ...
                          GPT_ChannelNumber_Type channelID)
{
  ...
  {
    GPTx->DMA_CNTL_CH.BF.DMA0_CH = channelID;
  }
  
  /* select a GPT channel for DMA channel 1 */
  if(GPT_DMA1 == dmaChannel)
  {
    GPTx->DMA_CNTL_CH.BF.DMA1_CH = channelID;
  }
}

/****************************************************************************//**
 * @brief      GPT ADC trigger enable
 *
 * @param[in]  gptID:  Select the GPT module
 * @param[in]  state: Enable / Disable 
 *
 ...
 *
 *******************************************************************************/
void GPT_ADCCmd(GPT_ID_Type gptID, FunctionalState state)
{
  gpt_reg_t * GPTx = (gpt_reg_t *)(gptAddr[gptID]);
  ...
  /* enable ADC trigger */
  GPTx->ADCT.BF.ADCT_EN = state;
}


/****************************************************************************//**
 * @brief      GPT ADC trigger config
 *
 * @param[in]  gptID:  Select the GPT module
 * @param[in]  channelID:  Channel ID.It indicates which channel can trigger ADC.
 * @param[in]  adcDelay: the delay after which a signal is generated to trigger ADC.
 *
 * @return none
 *
 *******************************************************************************/
void GPT_ADCConfig(GPT_ID_Type gptID, GPT_ChannelNumber_Type channelID, uint32_t adcDelay)
{	
  gpt_reg_t * GPTx = (gpt_reg_t *)(gptAddr[gptID]);
  ...
  GPTx->ADCT.BF.ADCT_CHSEL = channelID;
  
  /* set the delay time */
  GPTx->ADCT_DLY.BF.ADCT_DLY = adcDelay;
}
...
{   
  ...
  if(intType < GPT_CHANNEL_NUMBER)
  {
    if(intMsk == UNMASK )
    {
      GPTx->INT_MSK.WORDVAL &= (~(1<<(intType)));
    }
    else
    {
      GPTx->INT_MSK.WORDVAL |= (1<<(intType));
  ...
  if((intType >=  GPT_INT_CH0_ERR) && (intType <= GPT_INT_CH5_ERR))
  {
    if(intMsk == UNMASK )
    {
      GPTx->INT_MSK.WORDVAL &= (~(0x00000100<<(intType-6)));
    }
    else
    {
      GPTx->INT_MSK.WORDVAL |= (0x00000100<<(intType-6));
  ...
  switch(intType)
  {
    ...
    case GPT_INT_ALL:
      if(intMsk == UNMASK)
      {
        GPTx->INT_MSK.WORDVAL &= (~GPT_INT_ALL_MSK);
      }
      else
      {
        GPTx->INT_MSK.WORDVAL |= GPT_INT_ALL_MSK;
...
{  
  ...
  switch(status)
  {
    ...
    case GPT_STATUS_CNT_UPP:
      GPTx->STS.WORDVAL = GPT_INT_CNT_UPP_MSK;
      break;

    /* clear DMA CH0 status flag */
    case GPT_STATUS_DMA0_OF:
      GPTx->STS.WORDVAL = GPT_INT_DMA0_OVF_MSK;
      break;

    /* clear DMA CH1 status flag */
    case GPT_STATUS_DMA1_OF:
      GPTx->STS.WORDVAL = GPT_INT_DMA1_OVF_MSK;
      break;
      
     /* clear all the GPT status flags */
    case GPT_STATUS_ALL:
      GPTx->STS.WORDVAL = GPT_INT_ALL_MSK;
      break;
...
FlagStatus GPT_GetStatus(GPT_ID_Type gptID, GPT_Status_Type status)
{
  ...
  if(status < GPT_CHANNEL_NUMBER)
  {
    retStatus = ((GPTx->STS.WORDVAL) & (1<<(status)));
  }
    
  /* get channel error status */
  if((status >= 6) && (status < GPT_CHANNEL_NUMBER + 6))
  {
    retStatus = ((GPTx->STS.WORDVAL) & (1<<(status + 2)));
  }
  ...
  switch(status)
  {
    ...
    case GPT_STATUS_CNT_UPP:
      retStatus = GPTx->STS.BF.CNT_UPP_STS;
      break;

    /* get DMA Channel0 status */
    case GPT_STATUS_DMA0_OF:
      retStatus = GPTx->STS.BF.DMA0_OF_STS;
      break;

    /* get DMA Channel1 status */
    case GPT_STATUS_DMA1_OF:
      retStatus = GPTx->STS.BF.DMA1_OF_STS;
      break;

    /* get all status. If there is any interrupt, return SET, otherwise return RESET. */
    case GPT_INT_ALL:
      retStatus = (GPTx->STS.WORDVAL)&(0x03013F3F);
      break;
...
static void GPT_ChErrIRQHandler(INT_Peripher_Type intPeriph, GPT_ID_Type gptID, uint32_t chErrIntStatus)
{
  ...
  for(cnt = 0; cnt < GPT_CHANNEL_NUMBER; cnt++)
  {
    ...
    if((chErrIntStatus & 0x0100) != 0)
    {
      ...
      {
        /* Disable the interrupt if callback function is not setup */
        GPTx->INT_MSK.WORDVAL |= (0x00000100<<cnt) ;
      }
