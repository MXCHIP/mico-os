...
uint32_t *pm3_entryaddr __attribute__((used));
uint32_t *nvram_saved_sp_addr  __attribute__((used, section(".nvram")));
#ifdef CONFIG_HW_RTC
extern uint8_t tcalib;
#endif

/* This is used to detect wakeup form PM4*/
uint32_t pm4_sig __attribute__((section(".nvram_uninit")));
...
void update_time(uint32_t time_dur)
{
	...
	if (wakeup_reason & ERTC)
		os_update_tick_count(time_dur + 1);
	else
...
void asm_mcu_pm3()
{
	...
	(
		...
		"nvram_ret_addr_ptr : .word pm3_entryaddr\n"
...
static void mcu_pm4(uint32_t time_to_wakeup)
{
	...
	PMU_VbatBrndetCmd(DISABLE); // disable BOD
#endif
