...
#define iflash_address(x) (((x) & 0x1F000000)==0x1F000000)
#endif
...
#ifdef CONFIG_ENABLE_MXCHIP
	void* srccopy[NUM_DMA_CHANNELS];   /* copy if src address is iflash address, dma can't work with iflash directly */
#endif
...
#ifdef CONFIG_ENABLE_MXCHIP
	if (iflash_address(dmac->dma_cfg.srcDmaAddr)) {
		if (mdev_dma_p_data.srccopy[dma_ch])
			os_mem_free(mdev_dma_p_data.srccopy[dma_ch]);
		mdev_dma_p_data.srccopy[dma_ch] = 
			os_mem_alloc(dmac->dma_cfg.transfLength);
		memcpy((void*)mdev_dma_p_data.srccopy[dma_ch],
			   (void*)dmac->dma_cfg.srcDmaAddr,
			   dmac->dma_cfg.transfLength);
		dmac->dma_cfg.srcDmaAddr = (uint32_t)mdev_dma_p_data.srccopy[dma_ch];
	}
#endif
...
int dma_drv_wait_for_transfer_complete(mdev_t *dma, uint32_t timeout)
{
	...
	if (mdev_dma_p_data.srccopy[dma_ch]) {
		os_mem_free(mdev_dma_p_data.srccopy[dma_ch]);
		mdev_dma_p_data.srccopy[dma_ch] = NULL;
	}
#endif
