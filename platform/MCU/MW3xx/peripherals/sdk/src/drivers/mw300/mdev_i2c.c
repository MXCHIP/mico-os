...
static void i2c_drv_tx_abort_handler(int port_num)
{
	...
	/* Stub */
	I2C_GetTxAbortCause(port_num, &tx_abrt_reason);
	if (tx_abrt_reason & 0x0F) {
		p_data->no_ack_flag = 1;
	}
#endif	
}
...
#ifdef CONFIG_ENABLE_MXCHIP
/* Send read command to probe the slave I2C 
  * MAX retries.
  * 
  *  return 0=detect slave device, other not detect slave device
  */
int i2c_drv_probe(mdev_t *dev, uint32_t retries)
{
	int ret;
	I2C_CFG_Type *i2c_cfg;
	mdev_i2c_priv_data_t *priv_data_p;
	...
	do {
		...
		retries--;
	} while(priv_data_p->no_ack_flag == 1);
