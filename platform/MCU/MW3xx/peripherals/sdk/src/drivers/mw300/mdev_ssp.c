		...
		SSP_MS_Type mode, SSP_DMA dma, int cs, bool level)
{
	...
	case SSP_FRAME_SPI:
#ifdef CONFIG_ENABLE_MXCHIP
		SPI_Config(mdev_p->port_id, &ssp_data_p->spi_cfg);
#else
...
static void ssp_drv_mdev_init(uint8_t port_num)
{
	...
	ssp_data_p->rx_tout = OS_WAIT_FOREVER;
#ifdef CONFIG_ENABLE_MXCHIP
	ssp_data_p->spi_cfg.spiClkPhase = SPI_SCPHA_1;
	ssp_data_p->spi_cfg.spiClkPolarity = SPI_SCPOL_LOW;
	ssp_data_p->opened = 0;
#endif
...
#ifdef CONFIG_ENABLE_MXCHIP
mdev_t * ssp_get_dev(int port_num)
{
	mdev_t *mdev_p;
	sspdev_data_t *ssp_data_p;

	if (!IS_SSP_PERIPH(port_num))
		return NULL;
	/* Check ssp_id is initialized */
	mdev_p = mdev_get_handle(mdev_ssp_name[port_num]);
	if (!mdev_p) {
		SSP_LOG("Unable to open device %s\r\n",
			mdev_ssp_name[port_num]);
		return NULL;
	}
	ssp_data_p = (sspdev_data_t *) mdev_p->private_data;
	if (ssp_data_p->opened == 0) {
		SSP_LOG("Device %s is NOT opened\r\n",
			mdev_ssp_name[port_num]);
		return NULL;
	}
...
uint32_t ssp_drv_set_clk(SSP_ID_Type ssp_id, uint32_t freq)
{
	...
	uint16_t i, j, clk_div = 0, divider;
	...
	if (IS_SSP_DIVIDER(divider)) {
		clk_src = CLK_SYSTEM;
		clk_div = divider;
		ssp_data_p->freq = (uint32_t)((CHIP_SFLL_FREQ() / divider));
	...
	for (i = 0; i < sizeof(fvco) / sizeof(uint32_t); i++)
		for (j = 0; j < sizeof(pdiv); j++) {
			...
			if (IS_SSP_DIVIDER(divider)) {
				temp = (uint32_t)((fvco[i]/pdiv[j]) / divider);
...
#ifdef CONFIG_ENABLE_MXCHIP
int spi_drv_config(SSP_ID_Type ssp_id, SPI_Param_Type cfg)
{
	mdev_t *mdev_p;
	sspdev_data_t *ssp_data_p;
	...
	ssp_data_p->spi_cfg.spiClkPhase = cfg.spiClkPhase;
	ssp_data_p->spi_cfg.spiClkPolarity = cfg.spiClkPolarity;
	return WM_SUCCESS;
}
