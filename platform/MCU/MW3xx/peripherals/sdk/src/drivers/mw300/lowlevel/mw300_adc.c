 ...
 *  @{
 */
const static uint32_t adcAddr[2] = {ADC0_BASE};
...
void ADC_Init(ADC_ID_Type adcID, ADC_CFG_Type* adcConfigSet)
{
  ...
  /* yhb changed use PLL 200M clock */
...
void ADC_SetWarmupTime(ADC_ID_Type adcID, uint8_t us, uint8_t bypass)
{
	adc_reg_t * ADCx;
	...
	ADCx->INTERVAL.BF.WARMUP_TIME = us&0x1F;
	ADCx->INTERVAL.BF.BYPASS_WARMUP = bypass;
}
