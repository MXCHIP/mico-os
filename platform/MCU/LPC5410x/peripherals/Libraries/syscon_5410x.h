...
typedef struct {
	...
	__I  uint32_t RESERVED18b[3];		// /< Reserved (0x204 - 0x20c)
 ...
 *			a value of 1
 */
STATIC INLINE void Chip_FMC_SetFLASHAccess(SYSCON_FLASHTIM_T clks)
{
	uint32_t tmp;
	tmp = LPC_SYSCON->FLASHCFG & ~(0xF << 12);
	/* Don't alter lower bits */
	LPC_SYSCON->FLASHCFG = tmp | (clks << 12);
}
 ...
 */
extern uint32_t Is_Chip_SYSCON_PowerUp(uint32_t powerupmask);
