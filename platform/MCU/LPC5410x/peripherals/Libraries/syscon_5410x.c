...
void Chip_SYSCON_PowerUp_ZTE(uint32_t powerupmask)
{
...
uint32_t Is_Chip_SYSCON_PowerUp(uint32_t powerupmask)
{
	/* Get current power states. if the corresponding bit(s) are zero, it's powered up already. */
	return ( LPC_SYSCON->PDRUNCFG & powerupmask );
}

void Chip_SYSCON_NMI_Enable( uint32_t NMI_num )
{
